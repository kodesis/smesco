							...
							foreach ($bookings as $b) :
								...
								<tr>
									...
									<td>
										...
										if ($this->session->userdata('role_id') == '2' or $this->session->userdata('username') == 'krx0005') {
										?>
											<br><small class="text-muted"><em><?= $b->nama_pendaftar ?></em></small>
										<?php
										}
										...
										$status_badges = [
											'0' => ['text' => 'Pesanan Dibuat',           'color' => 'bg-secondary'],
											'1' => ['text' => 'Menunggu Penjemputan',      'color' => 'bg-yellow'],
											'2' => ['text' => 'Barang Dijemput',           'color' => 'bg-cyan'],
											'3' => ['text' => 'Tiba di Gudang Asal',       'color' => 'bg-teal'],
											'4' => ['text' => 'Dalam Pengiriman',          'color' => 'bg-azure'],
											'5' => ['text' => 'Tiba di Gudang Tujuan',     'color' => 'bg-orange'],
											'6' => ['text' => 'Terkirim ke Penerima',      'color' => 'bg-lime'],
										];

										if (isset($status_badges[$b->status_tracking])) {
											$status = $status_badges[$b->status_tracking];
										?>
											<br><span class="badge <?= $status['color']; ?> w-100"><?= $status['text']; ?></span>
										<?php
										}
										?>
									</td>
									...
									?>
										<td>
											<?= ($b->awb) ? $b->awb : "-" ?> <br>
											<em><?= $b->origin ?> - <?= $b->destination ?></em>
										</td>
									<?php
									} ?>
									<!-- <td><?= $b->commodity ?></td> -->
									<td class="text-end"><?= number_format($b->qty) ?> pc(s)</td>
									<td class="text-end"><?= number_format($b->chargeable) ?> kg(s)</td>
									<td class="text-end">Rp <?= number_format($b->nominal) ?></td>
									<td>
